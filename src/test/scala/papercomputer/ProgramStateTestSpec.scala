...
class ProgramStateTestSpec
  ...
  trait Fixture extends CommonFixtures {
    ...
    lazy val genProgramNoNextLine: Gen[(Program, LineNumber)] =
      genNonEmptyProgram.map(program => (program, program.keys.maxBy(_.value)))
    ...
    def genProgramStateForIncDecNoNextLine(incDecF: RegisterNumber => IncDec): Gen[(Program, LineNumber)] =
      for {
    ...
    def genProgramStateForFailingIncDec(incDecF: RegisterNumber => IncDec): Gen[(Program, LineNumber)] =
      for {
    ...
    lazy val genNonEmptyStack: Gen[Stack] = Gen.nonEmptyListOf(genProgramAndAnyContainedLine)
    ...
    def assertingIszF(expectedRn: RegisterNumber, expectedRs: Registers, resultingIsZero: Boolean): IszF =
      (rn: RegisterNumber) => {
        rn.shouldEqual(expectedRn)
        rs: Registers => {
      ...
      (registersOpsConfig, program, startLine: LineNumber, registers) =>
        whenever(!program.contains(startLine)) {
          val morPs: Mor[ProgramState] =
            ProgramState(registersOpsConfig, program, emptyStack, Some(startLine), registers)
  ...
  it should "fail with CannotRunAFinishedProgram" in new Fixture {
    def genPs: Gen[ProgramState] =
      for {
        ...
        program <- genProgram
        rs <- genRegisters
        programState = newProgramState(registersOpsConfig, program, emptyStack, None, rs)
      } yield programState

    forAll(genPs) { programState =>
      val morPs: Mor[ProgramState] = ProgramState.nextPs(programState)
  ...
  it should "return correct state if incF succeeds and next line exists" in new Fixture {
    forAll(genIncDecF, genIszF, genProgramForIncDecOK(Inc), genStack, genRegisters, genRegisters) {
      case (decF, iszF, (program, currentLine, registerNumber, firstNextLine), stack, oldRegisters, newRegisters) =>
  ...
  it should "fail if incF succeeds but next line does not exist" in new Fixture {
    forAll(genIncDecF, genIszF, genProgramStateForIncDecNoNextLine(Inc), genStack, genRegisters, genRegisters) {
      case (decF, iszF, (program, currentLine), stack, oldRegisters, newRegisters) =>
  ...
  it should "fail if provided incF fails" in new Fixture {
    forAll(genIncDecF, genIszF, genProgramStateForFailingIncDec(Inc), genStack, genRegisters) {
  ...
  it should "return correct state if decF succeeds and next line exists" in new Fixture {
    forAll(genIncDecF, genIszF, genProgramForIncDecOK(Dec), genStack, genRegisters, genRegisters) {
      case (incF, iszF, (program, currentLine, registerNumber, firstNextLine), stack, oldRegisters, newRegisters) =>
  ...
  it should "fail if decF succeeds but next line does not exist" in new Fixture {
    forAll(genIncDecF, genIszF, genProgramStateForIncDecNoNextLine(Dec), genStack, genRegisters, genRegisters) {
      case (incF, iszF, (program, currentLine), stack, oldRegisters, newRegisters) =>
  ...
  it should "fail if provided decF fails" in new Fixture {
    forAll(genIncDecF, genIszF, genProgramStateForFailingIncDec(Dec), genStack, genRegisters) {
  ...
  it should "return correct state if iszF returns true and second next line exists" in new Fixture {
    val genProgramStateForIszTrueOK: Gen[(Program, LineNumber, RegisterNumber, LineNumber)] = for {
    ...
    forAll(genIncDecF, genIncDecF, genProgramStateForIszTrueOK, genStack, genRegisters) {
      case (incF, decF, (program, currentLine, registerNumber, secondNextLine), stack, registers) =>
  ...
  it should "fail if iszF returns true and second next line does not exist" in new Fixture {
    ...
    forAll(genIncDecF, genIncDecF, genProgramStateForIszTrueNOK, genStack, genRegisters) {
  ...
  it should "return correct state if iszF returns false and next line exists" in new Fixture {
    ...
    forAll(genIncDecF, genIncDecF, genProgramStateForIszFalseOK, genStack, genRegisters) {
      case (incF, decF, (program, currentLine, firstNextLine), stack, registers) =>
  ...
  it should "fail if iszF returns false and next line does not exist" in new Fixture {
    ...
    forAll(genIncDecF, genIncDecF, genProgramStateForIszFalseNOK, genStack, genRegisters) {
      case (incF, decF, (program, currentLine), stack, registers) =>
        val iszF: IszF = _ => _ => Right(false)
        val config = RegistersOpsConfig(incF, decF, iszF)
        val programState = newProgramState(config, program, stack, Some(currentLine), registers)
  ...
  it should "fail if iszF fails" in new Fixture {
    ...
    forAll(genIncDecF, genIncDecF, genProgramStateForIszFails, genStack, genRegisters) {
      case (incF, decF, (program, currentLine), stack, registers) =>
  ...
  it should "return correct state for non-empty sub programs and next line exists" in new Fixture {
    val genProgramStateForNonEmptyPrg: Gen[(ProgramState, Program, LineNumber)] =
      for {
        (nep, currentLine, firstNextLine) <- genProgramNextLineExists
  ...
  it should "add a Right(ended program state) to the stream without calling next" in new StreamFixture {
    forAll(genNonEmptyStream, genFinishedProgramState) {
      (streamSoFar, finishedPs) =>
        ...
        newSize.shouldEqual(oldSize + 1)
        newStream.take(oldSize.toLong).toList.shouldEqual(streamSoFar.toList)
        newStream.last.toList.head.get.shouldEqual(finishedPs)
    }
  }

  it should "add a Right(non-ended program state)," +
    " then call the next function and add its ending program state result" in new StreamFixture {
    lazy val genEndingProgramState: Gen[Mor[ProgramState]] = for {
      morProgramState <- Gen.oneOf(genFinishedProgramState, Gen.const(Left(MessageDuringUnitTests)))
